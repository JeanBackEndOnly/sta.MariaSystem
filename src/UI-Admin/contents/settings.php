    ...
    ?>
        <form action="../../authentication/auth.php" method="post" enctype="multipart/form-data"
            ...
            <div class="profileSettings d-flex col-md-12 col-12 flex-wrap"
                style="height: auto !important; overflow-y: hidden !important;">
                <div
                    class="profilePicture h-100 col-md-4 col-12 d-flex flex-column justify-content-center align-items-center profileBG rounded-3 flex-wrap">
                    <?php if($profile) {?>
                    <label for="profilePreview"><img
                            src="../../authentication/uploads/<?= $LibrarianInfo["admin_picture"] ?>"
                            class="mediaProfile" alt="" style="width: 200px; height: 200px; border-radius: 50%;"
                            id="settingsProfile"></label>
                    <?php }else{ ?>
                    <img src="../../assets/image/users.png" alt=""
                        style="width: 200px; height: 200px; border-radius: 50%;">
                    <?php } ?>

                    <label for="user_profile" id="profilePreview"
                        class="cameraLabelMedia  rounded-3 m-0 p-2 d-flex align-items-center justify-content-center"
                        style="cursor: pointer; border: solid 1px #00000077;">
                        Change profile<img src="../../assets/image/ProfileInput.png" alt="" class="ms-2 p-0 "
                            style="width: 20px; height: 20px;">
                    </label>
                    <input type="hidden" name="current_profile_image" value="<?= $LibrarianInfo["admin_picture"] ?>">
                    <input type="file" name="user_profile" id="user_profile" class="form-control w-75"
                        style="display: none;" onchange="previewImageFaculty(event)">
                </div>
                <div
                    class="LibrarianInfo d-flex flex-row flex-wrap h-100 col-md-8 col-12 justify-content-start align-items-start noScrollBar">
                    <div class="m-1 col-md-5 col-11">
                        <label class="mb-0" for="">Surname</label>
                        <input type="text" name="admin_lastname" value="<?= $LibrarianInfo["admin_lastname"] ?>"
                            class="form-control">
                    </div>
                    <div class="m-1 col-md-5 col-11">
                        <label class="mb-0" for="">First Name</label>
                        <input type="text" name="admin_firstname" value="<?= $LibrarianInfo["admin_firstname"] ?>"
                            class="form-control">
                    </div>
                    <div class="m-1 col-md-5 col-11">
                        <label class="mb-0" for="">Middle Name</label>
                        <input type="text" name="admin_middlename" value="<?= $LibrarianInfo["admin_middlename"] ?>"
                            class="form-control">
                    </div>
                    <div class="m-1 col-md-5 col-11">
                        <label class="mb-0" for="">Name Extention</label>
                        <select name="admin_suffix" class="form-select">
                            <option value="" <?= empty($LibrarianInfo["admin_suffix"]) ? 'selected' : '' ?>>None
                            </option>
                            <option value="Jr." <?= $LibrarianInfo["admin_suffix"] == "Jr." ? 'selected' : '' ?>>Jr.
                            </option>
                            <option value="Sr." <?= $LibrarianInfo["admin_suffix"] == "Sr." ? 'selected' : '' ?>>Sr.
                            </option>
                            <option value="II" <?= $LibrarianInfo["admin_suffix"] == "II" ? 'selected' : '' ?>>II
                            </option>
                            <option value="III" <?= $LibrarianInfo["admin_suffix"] == "III" ? 'selected' : '' ?>>III
                            </option>
                            <option value="IV" <?= $LibrarianInfo["admin_suffix"] == "IV" ? 'selected' : '' ?>>IV
                            </option>
                        </select>
                    </div>
                    <div class="m-1 col-md-5 col-11">
                    ...
                    <div class="modal fade" id="updateProfile" tabindex="-1" aria-labelledby="borrowConfirmModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog modal-md modal-dialog-top">
                            <div class="modal-content">
                                <div class="modal-header bg-danger">
                                    <h5 class="modal-title text-white" id="borrowConfirmModalLabel">Update Confirmation
                                    </h5>
            ...
            <div class="modal-dialog">
                <form method="POST" action="../../authentication/auth.php" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="password" name="current_password" id="usernameConfim" class="form-control"
                            required>
                    </div>
                    <div class="modal-body">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="password" name="confirm_password" id="usernameConfim" class="form-control"
                            required>
                    </div>
                    <div class="modal-footer">
